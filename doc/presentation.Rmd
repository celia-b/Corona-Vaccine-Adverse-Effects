...
- Wrong column types automatically assigned by R &rightarrow; nanually assigned appropriate column types  
...
- True duplicates &rightarrow; distinct() 
...
- SYMPTOMVERSION1-5 columns are uninformative and dirty &rightarrow; select(c(VAERS_ID, SYMPTOM1, SYMPTOM2, SYMPTOM3, SYMPTOM4, SYMPTOM5))
...
- Some columns contain long string descriptions &rightarrow; Make tidy categorical variables 
  - Examples: ALLERGIES and OTHER_MEDS columns


- Unwanted columns &rightarrow; select(-c())
- NAs that should be interpreted as "no" &rightarrow; replace_na()
- Row duplications &rightarrow; distinct()
- Individuals who got more than one vaccine type (generates noise) distinct() _add_count(VAERS_ID) %>% filter(n==1) %>% select(-n)_
...
```{r death_v_symptoms_pval, echo = FALSE, out.width="50%", fig.show='hold'}
...
include_graphics("/cloud/project/results/sex_dist.png")
```
```{r echo=FALSE, out.width="30%", out.extra='style="float:right; padding:10px"'}
include_graphics("/cloud/project/results/vac_dist.png")
```
